...
def wizard2(request):
    ...
    #data, mejorar!!! usar GET para obtener los datos. GET es la única forma de que el HTML pueda pedir datos e
    ...
    if request.method == "POST":
        if edit_cat.is_valid() and wizardform.is_valid() and catform.is_valid():
            ...
            print("number of category ="+ str(len(Category.get_pk_categories(Category,cat))))
            #remove elements pk=pkeys
            return render_to_response("panel.html", locals(), context_instance = RequestContext(request))
